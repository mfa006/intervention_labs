...
tasks = [ 
    ...
    Configuration2D("End-effector configuration", np.array([1.0, 0.5, np.pi]).reshape(3,1), link=1),
...
plt.grid(True)
if len(tasks)==1:
    plt.savefig(f"e2_{tasks[0].name}.png")
else:
    plt.savefig(f"e2_{tasks[0].name}_{tasks[1].name}.png")
plt.show()